...
const f4 = () => {
    ...
    out.textContent = s4.has(input.value);
...
const f7 = () => {
    ...
    let arr = input.value.split([""[1]]);
    if (arr.length === new Set(arr).size && arr.length > 6) {
        out.textContent = 1;
    } else {
        out.textContent = 0;
    }
...
const f8 = () => {
    ar8 = [];
    let ar8_1 = Array.from(s8);
    for (let i = 0; i < ar8_1.length; i++) {
        if (ar8_1[i] > 5) {
            ar8.push(ar8_1[i]);
        }
    }
