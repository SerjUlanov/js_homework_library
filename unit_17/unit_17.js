...
// Task 5 ============================================
/*  Дан массив a5 = [3, 14, 15, 92,'6']. C помощью filter переберите массив и создайте a5_res, который содержит только четные числа из a5. Возвратите a5_res. Действия должны запускаться при вызове функции t5. Проверьте, чтобы функция корректно работала со строками, т.е. '6' не должно попасть в результирующий массив.  */

let a5 = [3, 14, 15, 92, '6'];

function t5() {
    a5_res = a5.filter(item => {
        if (item % 2 == 0 && typeof item == "number") return true;
    });
    return a5_res
}
...
function t6() {
    a6_res = a6.filter(item => {
        if (typeof item == "number" && item > 14) return true;
    });
    return a6_res
}
...
function t14() {
    let a14_res = []
    a14_res = a14.map(elem => {
        return elem.toLowerCase();
    });
    return a14_res.includes(a14_sym.toLowerCase());
