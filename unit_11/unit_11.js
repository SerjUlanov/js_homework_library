...
function f11() {
    ...
    out.textContent = d11.indexOf(+input.value);
}
...
function f15() {
    ...
    for (let i = 0; i < d15.length; i++) {
        if (d15.indexOf(+input.value) == -1) {
            d15.push(+input.value);
        }
    }
    showArr('.out-15', d15);
}
...
function f18() {
    ...
    out.textContent = d18.includes(input.value)
}
...
let d19 = ['Your', 'payment', 'method', 'will', 'automatically', 'be', 'charged', 'in', 'advance', 'every', "zzz"];
let maxString = '';
